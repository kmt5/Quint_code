...
exit();
    ...
    if($id==$user_id){echo '<p class ="ero2">自分のIDやで</p>'; exit();}
    if($data1 == Array()){echo '<p class ="ero2">IDに一致するユーザーが<br>見つかりませんでした</p>'; exit();}
    foreach ($result2 as $row){
      ...
      echo $row['nickname'].'</p>';
      echo '</div>';
    if(($data2 != Array()) or ($data3 != Array())){echo '<p class ="ero1">申請済みかフレンドです<p/>';}
